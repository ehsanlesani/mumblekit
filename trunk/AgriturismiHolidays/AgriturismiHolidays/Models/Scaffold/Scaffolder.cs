...
namespace Mumble.Web.StarterKit.Models.Scaffold
{
    ...
    public class Scaffolder
    {
        ...
        public void Save(Type entityType, NameValueCollection values)
        {
            ...
            foreach (var relationship in entityMetadata.Relationships)
            {
                ...
                if (relationship.To.RelationshipMultiplicity == RelationshipMultiplicity.One || relationship.To.RelationshipMultiplicity == RelationshipMultiplicity.ZeroOrOne)
                {
                    ...
                    {
                        ObjectContext.TryGetObjectByKey(new System.Data.EntityKey(String.Format("{0}.{1}", ObjectContext.DefaultContainerName, relationship.To.EntitySet), "Id", relatedEntityID), out relatedEntity);
                    }
                ...
                else if (relationship.To.RelationshipMultiplicity == RelationshipMultiplicity.Many)
                {
                    ...
                    if (controlValue != null)
                    {
                        ...
                        {
                            object relatedEntity = null;
                            if (ObjectContext.TryGetObjectByKey(new System.Data.EntityKey(String.Format("{0}.{1}", ObjectContext.DefaultContainerName, relationship.To.EntitySet), "Id", relatedEntityId), out relatedEntity))
                            {
        ...
        public void List(ViewContext viewContext, Type entityType)
        {
            ...
            //check if there are references and include into query load (eager)
            foreach (var column in configuration.Columns)
            {
                var count = entityMetadata.Relationships.Where(r => r.Name.Equals(column.Name)).Count();

                if (count > 0)
                {
                    entities = entities.Include(column.Name);
                }
            }

            ...
            MetadataDescriptor metadata = (from meta in _objectContext.MetadataWorkspace.GetItems(DataSpace.CSpace)
                                           ...
                                           select new MetadataDescriptor()
                                           {
                                               ...
                                               Relationships = from p in m.NavigationProperties
                                                               ...
                                                               {
                                                                   From = new RelationInfo() 
                                                                   { 
                                                                       MemberName = p.FromEndMember.Name, 
                                                                       RelationshipMultiplicity = p.FromEndMember.RelationshipMultiplicity,
                                                                       EntitySet = (from es in _objectContext.MetadataWorkspace.GetEntityContainer(_objectContext.DefaultContainerName, DataSpace.CSpace).BaseEntitySets
                                                                                    where es.ElementType.Name.Equals(((RefType)p.FromEndMember.TypeUsage.EdmType).ElementType.Name)
                                                                                    select es.Name).FirstOrDefault(),
                                                                       TypeName = ((RefType)p.FromEndMember.TypeUsage.EdmType).ElementType.Name
                                                                   },
                                                                   To = new RelationInfo() 
                                                                   { 
                                                                       MemberName = p.ToEndMember.Name, 
                                                                       RelationshipMultiplicity = p.ToEndMember.RelationshipMultiplicity ,
                                                                       EntitySet = (from es in _objectContext.MetadataWorkspace.GetEntityContainer(_objectContext.DefaultContainerName, DataSpace.CSpace).BaseEntitySets
                                                                                    where es.ElementType.Name.Equals(((RefType)p.ToEndMember.TypeUsage.EdmType).ElementType.Name)
                                                                                    select es.Name).FirstOrDefault(),
                                                                       TypeName = ((RefType)p.ToEndMember.TypeUsage.EdmType).ElementType.Name
